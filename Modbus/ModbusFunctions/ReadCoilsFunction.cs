        ...
        public override Dictionary<Tuple<PointType, ushort>, ushort> ParseResponse(byte[] response)
        {
            ...
            for (int i = 0; i < byteCount; i++)
            {
                ...
                for (int bit = 0; bit < 8; bit++)
                {
                    ...
                    ushort coilValue = (ushort)((currentByte >> bit) & 0x01);
                    ushort coilAddress = (ushort)(p.StartAddress + coilIndex);

                    result.Add(new Tuple<PointType, ushort>(PointType.DIGITAL_OUTPUT, coilAddress), coilValue);
                    coilIndex++;
                 }
